...
CanCan is an authorization library for Ruby on Rails which restricts what resources a given individual is allowed to access. All permissions are defined in a single location (the +Ability+ class) and not duplicated across controllers, views, and database queries.
...
The current individual's permissions can then be checked using the <tt>can?</tt> and <tt>cannot?</tt> methods in the view and controller.
...
The <tt>authorize!</tt> method in the controller will raise an exception if the individual is not able to perform the given action.
...
If the individual authorization fails, a <tt>CanCan::AccessDenied</tt> exception will be raised. You can catch this and modify its behavior in the +ApplicationController+.
